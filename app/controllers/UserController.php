    ...
    protected function requireUser()
{
    ...
    public function payment(){
        ...
        if($destination['quota'] < $quantity){
            $_SESSION['booking_error'] = 'Kuota tidak mencukupi untuk jumlah peserta yang Anda pilih.';
    ...
    public function payment_init(){
        ...
        if($booking_id){
            $this->destinationModel->decreaseQuota($destination_id, $people);
        }
    ...
    protected function prepareDestinationDetail($dest){
        ...
        $itineraries = [];
        if(!empty($dest['itinerary'])){
            $decoded = json_decode($dest['itinerary'], true);
            if(is_array($decoded)){
                $itineraries = $decoded;
            }
        }
        ...
        if(!empty($dest['facilities'])){
            ...
            if(is_array($decoded)){
                ...
                $facilities = array_filter(array_map('trim', explode(',', $dest['facilities'])));
            }
        } else {
            if($category === 'Gunung'){
